...
func SetupJoinFlags() (*flag.FlagSet, *JoinFlags) {
	...
	fs.StringVar(&flags.SchemaStrategy, "schema-strategy", "", "collision strategy for schemas (accept-left, accept-right, rename-left, rename-right, deduplicate, deduplicate-or-rename, fail)")
	...
	fs.Usage = func() {
		...
		Writef(fs.Output(), "  deduplicate-or-rename\n")
		Writef(fs.Output(), "                   Merge schemas that are interchangeable, rename the rest.\n")
		Writef(fs.Output(), "                   Never fails on a collision. Add --semantic-dedup to also\n")
		Writef(fs.Output(), "                   merge schemas that only become interchangeable once the\n")
