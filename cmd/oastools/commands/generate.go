...
func SetupGenerateFlags() (*flag.FlagSet, *GenerateFlags) {
	...
	fs.Usage = func() {
		Writef(fs.Output(), "Usage: oastools generate [flags] <file|url|->\n\n")
		Writef(fs.Output(), "Generate Go code from an OpenAPI specification.\n\n")
		Writef(fs.Output(), "Flags:\n")
		fs.PrintDefaults()
		Writef(fs.Output(), "\nExamples:\n")
		Writef(fs.Output(), "  oastools generate --client -o ./client openapi.yaml\n")
		Writef(fs.Output(), "  oastools generate --server -o ./server -p myapi openapi.yaml\n")
		Writef(fs.Output(), "  oastools generate --client --server -o ./api petstore.yaml\n")
		Writef(fs.Output(), "  oastools generate --types -o ./models https://example.com/api/openapi.yaml\n")
		Writef(fs.Output(), "  oastools generate --client --oauth2-flows -o ./client openapi.yaml\n")
		Writef(fs.Output(), "  oastools generate --client --credential-mgmt --security-enforce -o ./api openapi.yaml\n")
		Writef(fs.Output(), "  oastools generate --client --max-lines-per-file 1500 -o ./client large-api.yaml\n")
		Writef(fs.Output(), "  cat openapi.yaml | oastools generate --client -o ./client -\n")
		Writef(fs.Output(), "  oastools generate -s --client -o ./client openapi.yaml  # Include line numbers in issues\n")
		Writef(fs.Output(), "\nServer Generation Examples:\n")
		Writef(fs.Output(), "  oastools generate --server --server-all -o ./server openapi.yaml  # Full server with validation\n")
		Writef(fs.Output(), "  oastools generate --server --server-router=stdlib -o ./server openapi.yaml  # With router\n")
		Writef(fs.Output(), "  oastools generate --server --server-middleware --server-binder -o ./server openapi.yaml\n")
		Writef(fs.Output(), "\nPipelining:\n")
		Writef(fs.Output(), "  Use '-' as the file path to read the OpenAPI specification from stdin.\n")
		Writef(fs.Output(), "  Example: oastools convert -t 3.0.3 swagger.yaml | oastools generate --client -o ./client -\n")
		Writef(fs.Output(), "\nNotes:\n")
		Writef(fs.Output(), "  - At least one of --client, --server, or --types must be enabled\n")
		Writef(fs.Output(), "  - Types are always generated when --client or --server is enabled\n")
		Writef(fs.Output(), "  - Security helpers are generated by default when --client is enabled\n")
		Writef(fs.Output(), "  - Generated code uses Go idioms and best practices\n")
		Writef(fs.Output(), "  - Server interface is framework-agnostic\n")
	}
...
func HandleGenerate(args []string) error {
	...
	if specPath == StdinFilePath {
		...
		// Build generator options (23 base options + up to 7 server options)
