...
func (cg *oas2CodeGenerator) generateSplitTypes() error {
	...
	// Get all schemas
	var allSchemas []oas2SchemaEntry
	if cg.doc.Definitions != nil {
		for name, schema := range cg.doc.Definitions {
			if schema == nil {
				continue
			}
			allSchemas = append(allSchemas, oas2SchemaEntry{name: name, schema: schema})
			cg.schemaNames["#/definitions/"+name] = toTypeName(name)
		}
	}
...
func (cg *oas2CodeGenerator) generateSplitServer() error {
	...
	if err := cg.generateOAS2BaseServer(); err != nil {
		return err
	}
	...
	for _, group := range cg.splitPlan.Groups {
		...
		if err := cg.generateOAS2ServerGroupFile(group, opToPathMethod); err != nil {
...
func (cg *oas2CodeGenerator) generateOAS2BaseServer() error {
	var buf bytes.Buffer
