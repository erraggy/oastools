...
func (g *SecurityHelperGenerator) generateAPIKeyHelper(name string, scheme *parser.SecurityScheme) string {
	...
	switch scheme.In {
	case "header":
		buf.WriteString(fmt.Sprintf(`// With%sAPIKey sets the %s header for API key authentication.
`, funcName, keyName))
		if description != "" {
			buf.WriteString(fmt.Sprintf("// %s\n", cleanDescription(description)))
		}
		buf.WriteString(fmt.Sprintf(`func With%sAPIKey(key string) ClientOption {
...
`, funcName, keyName))
	...
	case "query":
		buf.WriteString(fmt.Sprintf(`// With%sAPIKeyQuery sets the %s query parameter for API key authentication.
`, funcName, keyName))
		if description != "" {
			buf.WriteString(fmt.Sprintf("// %s\n", cleanDescription(description)))
		}
		buf.WriteString(fmt.Sprintf(`func With%sAPIKeyQuery(key string) ClientOption {
	return func(c *Client) error {
		c.RequestEditors = append(c.RequestEditors, func(ctx context.Context, req *http.Request) error {
			q := req.URL.Query()
			q.Set(%q, key)
...
`, funcName, keyName))
	...
	case "cookie":
		buf.WriteString(fmt.Sprintf(`// With%sAPIKeyCookie sets the %s cookie for API key authentication.
`, funcName, keyName))
		if description != "" {
			buf.WriteString(fmt.Sprintf("// %s\n", cleanDescription(description)))
		}
		buf.WriteString(fmt.Sprintf(`func With%sAPIKeyCookie(key string) ClientOption {
	return func(c *Client) error {
		c.RequestEditors = append(c.RequestEditors, func(ctx context.Context, req *http.Request) error {
...
func (g *SecurityHelperGenerator) generateHTTPAuthHelper(name string, scheme *parser.SecurityScheme) string {
	...
	case "basic":
		buf.WriteString(fmt.Sprintf(`// With%sBasicAuth sets Basic authentication credentials.
`, funcName))
		if description != "" {
			buf.WriteString(fmt.Sprintf("// %s\n", cleanDescription(description)))
		}
		buf.WriteString(fmt.Sprintf(`func With%sBasicAuth(username, password string) ClientOption {
	return func(c *Client) error {
		c.RequestEditors = append(c.RequestEditors, func(ctx context.Context, req *http.Request) error {
...
`, funcName))
	...
	case "bearer":
		...
		buf.WriteString(fmt.Sprintf(`// With%sBearerToken sets the Bearer token for authentication.%s
`, funcName, formatComment))
		if description != "" {
			buf.WriteString(fmt.Sprintf("// %s\n", cleanDescription(description)))
		}
		buf.WriteString(fmt.Sprintf(`func With%sBearerToken(token string) ClientOption {
	return func(c *Client) error {
		c.RequestEditors = append(c.RequestEditors, func(ctx context.Context, req *http.Request) error {
...
`, funcName))
	...
	default:
		...
		buf.WriteString(fmt.Sprintf(`// With%sHTTPAuth sets the %s authentication header.
`, funcName, schemeName))
		if description != "" {
			buf.WriteString(fmt.Sprintf("// %s\n", cleanDescription(description)))
		}
		buf.WriteString(fmt.Sprintf(`func With%sHTTPAuth(credentials string) ClientOption {
	return func(c *Client) error {
		c.RequestEditors = append(c.RequestEditors, func(ctx context.Context, req *http.Request) error {
...
func (g *SecurityHelperGenerator) generateOAuth2Helper(name string, scheme *parser.SecurityScheme) string {
	...
	buf.WriteString(fmt.Sprintf(`// With%sOAuth2Token sets the OAuth2 bearer token for authentication.%s
`, funcName, scopesComment))
	if description != "" {
		buf.WriteString(fmt.Sprintf("// %s\n", cleanDescription(description)))
	}
	buf.WriteString(fmt.Sprintf(`func With%sOAuth2Token(token string) ClientOption {
	return func(c *Client) error {
		c.RequestEditors = append(c.RequestEditors, func(ctx context.Context, req *http.Request) error {
			req.Header.Set("Authorization", "Bearer "+token)
			return nil
...
func (g *SecurityHelperGenerator) generateOpenIDConnectHelper(name string, scheme *parser.SecurityScheme) string {
	...
	buf.WriteString(fmt.Sprintf(`// With%sToken sets the OpenID Connect token for authentication.%s
`, funcName, urlComment))
	if description != "" {
		buf.WriteString(fmt.Sprintf("// %s\n", cleanDescription(description)))
	}
	buf.WriteString(fmt.Sprintf(`func With%sToken(token string) ClientOption {
	return func(c *Client) error {
		c.RequestEditors = append(c.RequestEditors, func(ctx context.Context, req *http.Request) error {
			req.Header.Set("Authorization", "Bearer "+token)
			return nil
