...
func TestCopyTags(t *testing.T) {
	tests := []struct {
		...
		{
			name: "single tag without external docs",
			tags: []*parser.Tag{
				{
					Name:        "users",
		...
		{
			name: "multiple tags with nil element",
			tags: []*parser.Tag{
				{Name: "users"},
				nil,
				{Name: "products"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(copied) != len(tt.tags) {
				t.Errorf("Length mismatch: got %d, want %d", len(copied), len(tt.tags))
			}

			for i, tag := range tt.tags {
				if tag == nil {
					if copied[i] != nil {
						t.Errorf("Tag %d should be nil", i)
					}
					continue
				}

				if copied[i] == tag {
					t.Errorf("Tag %d should be a different pointer", i)
				}

				if !reflect.DeepEqual(copied[i], tag) {
					t.Errorf("Tag %d content mismatch", i)
				}

				if tag.ExternalDocs != nil && copied[i].ExternalDocs == tag.ExternalDocs {
					t.Errorf("Tag %d ExternalDocs should be a different pointer", i)
				}
			}
		})
	}
...
func TestCopyExternalDocs(t *testing.T) {
	tests := []struct {
		name string
		docs *parser.ExternalDocs
	}{
		{
			name: "nil docs",
			docs: nil,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			copied := copyExternalDocs(tt.docs)
			...
			if !reflect.DeepEqual(copied, tt.docs) {
...
func TestCopyStringSlice(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			copied := copyStringSlice(tt.slice)
			...
			if len(copied) != len(tt.slice) {
				t.Errorf("Length mismatch: got %d, want %d", len(copied), len(tt.slice))
			}

			if !reflect.DeepEqual(copied, tt.slice) {
				t.Error("Content mismatch")
			}

			// Verify it's a different slice (different pointer if non-empty)
			if len(tt.slice) > 0 && &copied[0] == &tt.slice[0] {
				t.Error("Copy should be a different slice")
			}
		})
	}
}
