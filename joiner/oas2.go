...
func (j *Joiner) joinOAS2Documents(docs []parser.ParseResult) (*JoinResult, error) {
	...
	if !result.scope.empty() || j.needsDeclarationOrder(len(joined.Definitions)) {
	...
	if j.config.SemanticDeduplication && len(joined.Definitions) > 1 {
		err := j.deduplicateSchemas(dedupeTarget{
			document: joined,
			schemas:  joined.Definitions,
			version:  joined.OASVersion,
			owner:    owner,
			copied:   copied,
			section:  sectionDefinition,
			apply:    func(schemas map[string]*parser.Schema) { joined.Definitions = schemas },
		}, result)
		if err != nil {
			return nil, err
		}
...
func (j *Joiner) mergeOAS2Definitions(joined, source *parser.OAS2Document, ctx documentContext, result *JoinResult, graphs *refGraphs) error {
	...
	for name, schema := range source.Definitions {
		...
		if j.config.AlwaysApplyPrefix && sourcePrefix != "" {
			...
			result.AddWarning(NewNamespacePrefixWarning(name, effectiveName, sectionDefinition, ctx.filePath, line, col))
		}
		...
		if _, exists := joined.Definitions[effectiveName]; exists {
			...
			case StrategyDeduplicateEquivalent:
				...
				if mode != EquivalenceModeNone {
					...
					if eqResult.Equivalent {
						...
						line, col := j.getLocation(ctx.filePath, fmt.Sprintf("$.definitions.%s", name))
						result.AddWarning(NewSchemaDedupWarning(effectiveName, sectionDefinition, ctx.filePath, line, col))
			...
			case StrategyRenameLeft:
				...
				result.AddWarning(NewSchemaRenamedWarning(effectiveName, newName, sectionDefinition, leftOrigin.filePath, line, col, true))
			...
			case StrategyRenameRight:
				...
				line, col := j.getLocation(ctx.filePath, fmt.Sprintf("$.definitions.%s", name))
				result.AddWarning(NewSchemaRenamedWarning(effectiveName, newName, sectionDefinition, ctx.filePath, line, col, false))
