...
ifeq ("$(shell command -v gotestsum)", "")
	go test -v -race -coverprofile=coverage.txt -covermode=atomic -parallel=4 -run='Test' -skip='Fuzz' ./...
else
	gotestsum --format testname -- -v -coverprofile=coverage.txt -covermode=atomic -timeout=60m -race -failfast -parallel=4 -run='Test' -skip='Fuzz' ./...
endif
