...
ifeq ("$(shell command -v gotestsum)", "")
	GORACE="halt_on_error=1" GOMAXPROCS=1 go test -v -race -short -timeout=20m -p=1 -parallel=1 -skip='^Fuzz' ./...
else
	GORACE="halt_on_error=1" GOMAXPROCS=1 gotestsum --format testname -- -v -race -short -timeout=20m -failfast -p=1 -parallel=1 -skip='^Fuzz' ./...
endif
